...
class PageLogin(unittest.TestCase):
    def setUp(self):
        logging.debug("================================== " + Helper.Helper_common.get_func_name() + " ==================================")
    ...
    def test_UI_LG_03(self):
        ...
        driver.execute_script('''document.title="''' + self._testMethodName + '''"''')
        try:
            self.assertNotEqual(ipv6_global, 'http://[unknown]')
            self.list_steps.append('\n[Pass] 3. Ipv6 global existed')
            ...
            self.list_steps.append('\n[Fail] 3. Ipv6 global was not existed')
            list_steps_fail.append('3. Ipv6 global was not existed ')
    ...
    def test_UI_LG_05(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_quick_setup)
            self.list_steps.append('\n[Pass] 2. Quick setup URL after wait sension: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Quick setup URL after wait sension: ' + driver.current_url)
            list_steps_fail.append('2. Quick setup URL after wait sension')
        time.sleep(3)
        ...
        try:
            self.assertEqual(actual_password, keys_send)
            self.list_steps.append('\n[Pass] 3. Change password after sension.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3. Change password after sension: \nActual: %s. \nExpected: %s.' %(actual_password, keys_send))
            list_steps_fail.append('3. Change password after sension')
        self.assertListEqual(list_steps_fail, [], '[UI_LG_05] Assertion fail')
        time.sleep(120)
    ...
    def test_UI_LG_06(self):
        ...
        sample_input = '\; ls -l\;'
        for i in range(1, 6):
            Helper.Helper_common.test_login(driver, self, ipv4, user=sample_input, pass_word=sample_input)
            time.sleep(1)
            # Check red error
            error = driver.find_elements_by_css_selector('.f-wrap.error')
            try:
                self.assertNotEqual(len(error), 0)
                self.list_steps.append("\n[Pass] 2.1."+str(i)+" Check Red text Error appear")
            except AssertionError:
                self.list_steps.append("\n[Fail] 2.1."+str(i)+" Check Red text Error appear")
                list_steps_fail.append("\n2.1."+str(i)+" Check Red text Error appear but it were not appear")
            time.sleep(2)
            # # Check response from API login
            # api = Helper.Helper_common.check_current_request(driver, 'api/v1/gateway/users/login')
            # actual = [api['error']['code']]
            # expected = [422]
            # try:
            #     self.assertListEqual(actual, expected)
            #     self.list_steps.append("\n[Pass] 2.2."+str(i)+" Check body of api login")
            # except AssertionError:
            #     self.list_steps.append("\n[Fail] 2.2."+str(i)+" Check body of api login. \nActual: %s. \nExpected: %s" %(actual, expected))
            #     list_steps_fail.append("\n2.2."+str(i)+" Check body of api login. \nActual: %s. \nExpected: %s" %(actual, expected))
        ...
        try:
            self.assertNotEqual(len(error), 0)
            self.list_steps.append("\n[Pass] 3.1 Check Red text Error appear with correct info")
            # api = Helper.Helper_common.check_current_request(driver, 'api/v1/gateway/users/login')
            # actual = [api['error']['code']]
            # expected = [422]
            # try:
            #     self.assertListEqual(actual, expected)
            #     self.list_steps.append("\n[Pass] 3.2 Check body of api login with correct info")
            # except AssertionError:
            #     self.list_steps.append(
            #         "\n[Fail] 3.2 Check body of api login with correct info. \nActual: %s. \nExpected: %s" % (
            #         actual, expected))
            #     list_steps_fail.append(
            #         "\n3.2 Check body of api login with correct info. \nActual: %s. \nExpected: %s" % (
            #         actual, expected))
        except AssertionError:
            self.list_steps.append("\n[Fail] 3.1 Check Red text Error appear with correct info but Login Successfully")
            list_steps_fail.append("\n3.1 Check Red text Error appear with correct info but Login Successfully")
        time.sleep(2)
        ...
        try:
            self.assertNotEqual(len(error), 0)
            self.list_steps.append("\n[Pass] 4.1 Check Red text Error appear with incorrect info")
        except AssertionError:
            self.list_steps.append("\n[Fail] 4.1 Check Red text Error appear with incorrect info")
            list_steps_fail.append("\n4.1 Check Red text Error appear but it were not appear with incorrect info")
        time.sleep(2)
        # # Check response from API login
        # api = Helper.Helper_common.check_current_request(driver, 'api/v1/gateway/users/login')
        # actual = [api['error']['code']]
        ...
        # try:
        #     self.assertListEqual(actual, expected)
        #     self.list_steps.append("\n[Pass] 4.2 Check body of api login with correct info")
        # except AssertionError:
        #     self.list_steps.append(
        #         "\n[Fail] 4.2 Check body of api login with incorrect info. \nActual: %s. \nExpected: %s" % (
        #         actual, expected))
        #     list_steps_fail.append(
        #         "\n4.2 Check body of api login with incorrect info. \nActual: %s. \nExpected: %s" % (
        #         actual, expected))
        time.sleep(120)
        Helper.Helper_common.login(driver, self, ipv4)
        ...
        try:
            ...
            self.list_steps.append('\n[Pass] 5.Login success: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 5.Login success ' + driver.current_url)
    ...
    def test_UI_LG_07(self):
        logging.debug(
            "================================== " + Helper.Helper_common.get_func_name() + " ==================================")
        logging.debug("")
        self.def_name = Helper.Helper_common.get_func_name()
        list_steps_fail = []
        self.list_steps = []
        ...
        sample_input = '\; ls -l\;'
        for i in range(1, 6):
            Helper.Helper_common.test_login(driver, self, ipv6_global, user=sample_input, pass_word=sample_input)
            time.sleep(5)
            # Check red error
            error = driver.find_elements_by_css_selector('.f-wrap.error')
            try:
                self.assertNotEqual(len(error), 0)
                self.list_steps.append("\n[Pass] 2.1. " + str(i) + " Check Red text Error")
            except AssertionError:
                self.list_steps.append("\n[Fail] 2.1. " + str(i) + " Check Red text Error")
                list_steps_fail.append("\n2.1. " + str(i) + " Check Red text Error but it were not disappear")
            time.sleep(2)
        ...
        try:
            ...
            self.list_steps.append("\n[Pass] 3.1 Check Red text Error appear with incorrect info")
        except AssertionError:
            self.list_steps.append("\n[Fail] 3.1 Check Red text Error appear with incorrect info")
            list_steps_fail.append("\n3.1 Check Red text Error appear but it were not appear with incorrect info")
        ...
        time.sleep(120)
        Helper.Helper_common.login(driver, self, ipv6_global)
        expected_url = ipv6_global + '/#page-quick-setup'
        time.sleep(5)
        try:
            self.assertEqual(driver.current_url, expected_url)
            self.list_steps.append('\n[Pass] 5.Login success: ' + driver.current_url)
            ...
            self.list_steps.append('\n[Fail] 5.Login success ' + driver.current_url)
            list_steps_fail.append('5. URL QS wrong: ' + driver.current_url)
        self.assertListEqual(list_steps_fail, [], '[UI_LG_07] Assertion wrong')
...
class PageQuickSetup(unittest.TestCase):
    ...
    def test_UI_QS_02(self):
        ...
        try:
            ...
            self.list_steps.append('\n[Fail] 3. Display Name wrong: \nAcutal: %s. \nExpected: %s.' %(str(actual_name), str(keys_name[:31])))
            list_steps_fail.append('3.  Display Name wrong: \nAcutal: %s. \nExpected: %s.' %(str(actual_name), str(keys_name[:31])))
...
class PageStatusMTAStatus(unittest.TestCase):
    ...
    def test_UI_SMS_01(self):
        ...
        try:
            ...
            self.list_steps.append('\n[Fail] 3. Check List Initialization Procedure Status:\nActual: %s. \nExpected: %s.' %(str(status_actual)), str(status_expected_convert))
            list_steps_fail.append('3. Check List Initialization Procedure Status:\nActual: %s. \nExpected: %s.' %(str(status_actual)), str(status_expected_convert))
...
class PageNetworkBasicSettings(unittest.TestCase):
    def setUp(self):
        ...
        self.driver.execute_script('''document.title="''' + self._testMethodName + '''"''')
    ...
    def test_UI_NBS_01(self):
        ...
        try:
            ...
            list_steps_fail.append(
                '2.2 API LAN Network Basic setting page wrong: \nActual: %s. \nExpected: %s' % (str(actual), str(expected)))
    ...
    def test_UI_NBS_05(self):
        ...
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/network/wan')
        expected = ['bridge']
        actual = [api_lan['interfaces'][0]['switchMode']]

        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of switchMode.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi switchMode. \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Value of APi switchMode: \nActual: %s.  \nExpected: %s' % (str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_NBS_05] Assertion wrong')
...
class PageNetworkDHCP(unittest.TestCase):
    ...
    def test_UI_ND_03(self):
        ...
        try:
            ...
            list_steps_fail.append('1. Ping DUT fail')
            pass
        ...
        try:
            ...
            list_steps_fail.append('[UI_ND_03] 1. Test PC IP is not changed')
            pass
        ...
        try:
            ...
            list_steps_fail.append('[UI_ND_03] 7. Ping DUT fail')
            pass
    ...
    def test_UI_ND_04(self):
        ...
        old_dns_server = re.findall("DNS Servers . . . . . . . . . . . : (192.168.[0-9]*.[0-9]*)", ipconfig_pc)[0]
        ...
        ipconfig_pc = str(subprocess.check_output('ipconfig /all', shell=True))
        current_dns_server = re.findall("DNS Servers . . . . . . . . . . . : (192.168.[0-9]*.[0-9]*)", ipconfig_pc)[0]
        try:
    ...
    def test_UI_ND_06(self):
        ...
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of Local Address.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of Local Address. \nActual: %s. \nExpected: %s' %(str(actual), str(expected)))
            list_steps_fail.append('3. Value of Local Address: \nActual: %s.  \nExpected: %s' %(str(actual), str(expected)))
        ...
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of Enable DHCP server (If SIM >>True), Starting local address, Number of CPE, Lease time.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of Enable DHCP server (If SIM >>True), Starting local address, Number of CPE, Lease time. \nActual: %s. \nExpected: %s' %(str(actual), str(expected)))
            list_steps_fail.append('3. Value of Enable DHCP server (If SIM >>True), Starting local address, Number of CPE, Lease time: \nActual: %s.  \nExpected: %s' %(str(actual), str(expected)))
        ...
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of APi ipAddr, active, StartIp, EndIp, Lease time.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi ipAddr, active, StartIp, EndIp, Lease time. \nActual: %s. \nExpected: %s' %(str(actual), str(expected)))
            list_steps_fail.append('3. Value of APi ipAddr, active, StartIp, EndIp, Lease time: \nActual: %s.  \nExpected: %s' %(str(actual), str(expected)))
...
class PageNetworkDHCPv6(unittest.TestCase):
    ...
    def test_UI_NDv6_08(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url)
            self.list_steps.append('\n[Pass] 2. Check Page Network DHCPv6: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check Page Network DHCPv6: ' + driver.current_url)
            list_steps_fail.append('2.URL page Network DHCPv6 display wrong' + driver.current_url)
        time.sleep(3)

        # Value of ATIVAR, ATIVAR RAPID COMMIT, DESATIVAR DHCPV6 STATELESS are checked; Number of Address
        ativar = Helper.Helper_common.check_radio_tick(driver, '#ativar')
        number_addr = driver.find_element_by_css_selector('#numero-de-endereços').get_attribute('value')
        ativar_rapid = Helper.Helper_common.check_radio_tick(driver, '#ativar-comprometimento-rapido')
        dhcpv6 = Helper.Helper_common.check_radio_tick(driver, '#dhcpv6-stateless')
        actual = [ativar, number_addr, ativar_rapid, dhcpv6]
        expected = [True, '255', True, True]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append(
                '\n[Pass] 3. Value of ATIVAR, ATIVAR RAPID COMMIT, DESATIVAR DHCPV6 STATELESS are checked; Number of Address')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of ATIVAR, ATIVAR RAPID COMMIT, DESATIVAR DHCPV6 STATELESS are checked; Number of Address: \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Value of ATIVAR, ATIVAR RAPID COMMIT, DESATIVAR DHCPV6 STATELESS are checked; Number of Address: \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/network/lan')
        expected = ['stateful', 255, True]
        actual = [api_lan['ipv6']['mode'],
                  api_lan['ipv6']['dhcpv6']['numberOfAddresses'],
                  api_lan['ipv6']['dhcpv6']['rapidCommit']]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of APi: mode, numberOfAddresses, rapidCommit.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi: mode, numberOfAddresses, rapidCommit: \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check Value of APi: mode, numberOfAddresses, rapidCommit: \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_NDv6_08] Assertion wrong')
...
class PageNetworkLanV6(unittest.TestCase):
    ...
    def test_UI_NLv6_01(self):
        ...
        url_req = ipv4 + '/api/v1/gateway/devices?interface=lan-2.4g-5g'
        token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]

        api_lan_ipv6 = Helper.Helper_common.call_get_api_token(url_req, token)
        expected_lan_ipv6 = [api_lan_ipv6[0][0]['ipv6LinkLocalAddress'],
                             api_lan_ipv6[0][0]['macAddress'],
                             api_lan_ipv6[0][0]['ipv6Reachable']]
        time.sleep(3)
        ...
        expected_lan_ipv6 = [i.strip() for i in expected_lan_ipv6]
        try:
        ...
        self.assertListEqual(list_steps_fail, [], '[UI_NLv6_01] Assertion fail')
...
class PageNetworkDNSv4(unittest.TestCase):
    ...
    def test_UI_NDv4_04(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url)
            self.list_steps.append('\n[Pass] 2. Check Page Network DNSv4: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check Page Network DNSv4: ' + driver.current_url)
            list_steps_fail.append('2.URL page Network DNSv4 display wrong' + driver.current_url)
        time.sleep(3)

        # Check dns_server is unchecked, dns_primario & dns-secundario are disabled
        dns_server = Helper.Helper_common.check_radio_tick(driver, '#dns-active')
        dns_primario = driver.find_element_by_css_selector('#dns-primario').get_attribute('disabled')
        dns_secundario = driver.find_element_by_css_selector('#dns-secundario').get_attribute('disabled')
        actual = [dns_server, dns_primario, dns_secundario]
        expected = [False, 'true', 'true']
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append(
                '\n[Pass] 3. Check dns_server is unchecked, dns_primario & dns-secundario are disabled.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check dns_server is unchecked, dns_primario & dns-secundario are disabled: \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check dns_server is unchecked, dns_primario & dns-secundario are disabled: \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/network/wan')
        expected = [True, '', ' ']
        actual = [api_lan['interfaces'][0]['ipv4']['dnsAuto'],
                  api_lan['interfaces'][0]['ipv4']['dnsServer1'],
                  api_lan['interfaces'][0]['ipv4']['dnsServer2']
                  ]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of APi dnsAuto, dnsServer1, dnsServer2.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi dnsAuto, dnsServer1, dnsServer2. \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check Value of APi dnsAuto, dnsServer1, dnsServer2. \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_NDv4_04] Assertion wrong')
...
class PageNetworkWanConfiguration(unittest.TestCase):
    ...
    def test_UI_NWC_04(self):
        ...
        # Check auto get ip address is checked, use static ip address is unchecked
        auto_get_addr = Helper.Helper_common.check_radio_tick(driver, '#dynamic-mode')
        sta_ip_addr = Helper.Helper_common.check_radio_tick(driver, '#static-mode')
        actual = [auto_get_addr, sta_ip_addr]
        expected = [True, False]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append(
                '\n[Pass] 3. Check auto get ip address is checked, use static ip address is unchecked.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check auto get ip address is checked, use static ip address is unchecked: \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check auto get ip address is checked, use static ip address is unchecked: \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/network/wan')
        expected = ['dynamic']
        actual = [api_lan['interfaces'][0]['ipv4']['mode']]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of APi IPv4 mode.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi IPv4 mode. \nActual: %s. \nExpected: %s' % (str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check Value of APi IPv4 mode. \nActual: %s.  \nExpected: %s' % (str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_NWC_04] Assertion wrong')
...
class PageAdvancedDMZHost(unittest.TestCase):
    ...
    def test_UI_ADH_05(self):
        ...
        # Check DMZ IP Address
        dmz_ip_addr = driver.find_element_by_css_selector("#dmz-ip-address").get_attribute("value")
        actual = [dmz_ip_addr]
        expected = ['192.168.0.2']
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check DMZ IP Address.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check DMZ IP Address: \nActual: %s. \nExpected: %s' % (str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check DMZ IP Address: \nActual: %s.  \nExpected: %s' % (str(actual), str(expected)))

        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/dmz')
        expected = [False, '192.168.0.2']
        actual = [api_lan['active'],
                  api_lan['destination']]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of APi active & destination.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi active & destination. \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append('3. Check Value of APi active & destination. \nActual: %s.  \nExpected: %s' % (
            str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_ADH_05] Assertion wrong')
...
class PageAdvancedDDNSService(unittest.TestCase):
    ...
    def test_UI_AD_01(self):
        ...
        try:
            self.assertListEqual(actual_api, expected)
            self.list_steps.append('[Pass] 5. Check values return from API')
        except AssertionError:
            self.list_steps.append('[Fail] 5. Check values return from API')
    ...
    def test_UI_AD_02(self):
        ...
        self.assertListEqual(list_steps_fail, [], '[UI_AD_02] Assertion Enable DDNS service fail')
    ...
    def test_UI_AD_04(self):
        ...
        # Check display only ADICIONAR button
        adicionar_btn = driver.find_element_by_css_selector("label.button.icon").text
        actual = [adicionar_btn[:9]]
        expected = ['ADICIONAR']
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check display only ADICIONAR button')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check display only ADICIONAR button: \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check display only ADICIONAR button: \nActual: %s.  \nExpected: %s' % (str(actual), str(expected)))

        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/ddns')
        expected = [False, 0, '', '', '', '']
        actual = [api_lan['active'],
                  api_lan['configuration']['currentProvider'],
                  api_lan['configuration']['username'],
                  api_lan['configuration']['password'],
                  api_lan['configuration']['hostname'],
                  api_lan['configuration']['token'],
                  ]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append(
                '\n[Pass] 3. Check Value of APi active, currentProvider, username, password, hostname, token.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi active, currentProvider, username, password, hostname, token. \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check Value of APi active, currentProvider, username, password, hostname, token. \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_AD_04] Assertion wrong')
...
class PageAdvancedOptionAdvanced(unittest.TestCase):
    ...
    def test_UI_AOD_01(self):
        ...
        try:
            ...
            self.list_steps.append('[Pass] 3. Check value when enable Wan Blocking: ' + str(api_networkOption['wanBlocking']))
        except AssertionError:
            self.list_steps.append('[Fail] 3. Check value when enable Wan Blocking: '+ str(api_networkOption['wanBlocking']))
            list_steps_fail.append('3. Check value when enable Wan Blocking wrong: '+ str(api_networkOption['wanBlocking']))
            pass
    ...
    def test_UI_AOD_08(self):
        ...
        # Check Lock WAN, IPSEC PASS THROUGH, PPTP PASS THROUGH, REMOTE CONFIGURATION MANAGEMENT, MULTICAST ATIVAR, UPNP ATIVAR

        lock_wan = Helper.Helper_common.check_radio_tick(driver, '#wanLock')
        ipsec_pass_through = Helper.Helper_common.check_radio_tick(driver, '#ipsecPassThrough')
        pptp_pass_through = Helper.Helper_common.check_radio_tick(driver, '#pptpPassThrough')
        remote_config_management = Helper.Helper_common.check_radio_tick(driver, '#remoteConfigManagement')
        multicast_active = Helper.Helper_common.check_radio_tick(driver, '#multicastActive')
        upnp_active = Helper.Helper_common.check_radio_tick(driver, '#upnpActive')

        actual = [lock_wan, ipsec_pass_through, pptp_pass_through, remote_config_management, multicast_active,
                  upnp_active]
        expected = [True, False, False, False, True, False]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append(
                '\n[Pass] 3. Check button: Lock WAN, IPSEC PASS THROUGH, PPTP PASS THROUGH, REMOTE CONFIGURATION MANAGEMENT, MULTICAST ATIVAR, UPNP ATIVAR')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check button:  Lock WAN, IPSEC PASS THROUGH, PPTP PASS THROUGH, REMOTE CONFIGURATION MANAGEMENT, MULTICAST ATIVAR, UPNP ATIVAR \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check button Lock WAN, IPSEC PASS THROUGH, PPTP PASS THROUGH, REMOTE CONFIGURATION MANAGEMENT, MULTICAST ATIVAR, UPNP ATIVAR \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/networkOption')
        expected = [True, False, False, False, True, False]
        actual = [api_lan['wanBlocking'],
                  api_lan['ipsecPassthrough'],
                  api_lan['pptpPassthrough'],
                  api_lan['l2tpPassthrough'],
                  api_lan['multicast'],
                  api_lan['zeroConfiguration']
                  ]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append(
                '\n[Pass] 3. Check Value of APi wanBlocking, ipsecPassthrough, pptpPassthrough, l2tpPassthrough, multicast, zeroConfiguration.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi wanBlocking, ipsecPassthrough, pptpPassthrough, l2tpPassthrough, multicast, zeroConfiguration. \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check Value of APi wanBlocking, ipsecPassthrough, pptpPassthrough, l2tpPassthrough, multicast, zeroConfiguration. \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/upnp')
        expected = [False]
        actual = [api_lan['active']]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of APi active.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi active. \nActual: %s. \nExpected: %s' % (str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check Value of APi active. \nActual: %s.  \nExpected: %s' % (str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_AOD_08] Assertion wrong')
...
class PageSecurityFirewallBasic(unittest.TestCase):
    ...
    def test_UI_SFB_11(self):
        ...
        value = driver.find_element_by_css_selector("select[name='ipv4']").get_attribute('value')
        ip_v4 = driver.find_element_by_css_selector("select[name='ipv4'] > option[value='" + value + "']").text

        value = driver.find_element_by_css_selector("select[name='ipv6']").get_attribute('value')
        ip_v6 = driver.find_element_by_css_selector("select[name='ipv6'] > option[value='" + value + "']").text

        fragmented_ip = Helper.Helper_common.check_radio_tick(driver, '#fragmentedIp')
        port_scan_detect = Helper.Helper_common.check_radio_tick(driver, '#portScanDetect')
        ip_flood = Helper.Helper_common.check_radio_tick(driver, '#ipFlood')

        actual = [ip_v4, ip_v6, fragmented_ip, port_scan_detect, ip_flood]
        expected = ['Baixo', 'Ligado', False, False, False]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append(
                '\n[Pass] 3. Check value: Firewall ipv4, Firewall ipv6; Check button: fragmentedIp, portScanDetect, ipFlood')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check value: Firewall ipv4, Firewall ipv6; Check button: fragmentedIp, portScanDetect, ipFlood: \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check value: Firewall ipv4, Firewall ipv6; Check button: fragmentedIp, portScanDetect, ipFlood: \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/security/firewall')
        expected = [True, 'low', False, False, False, True]
        actual = [api_lan['ipv4']['active'],
                  api_lan['ipv4']['level'],
                  api_lan['ipv4']['blocks']['fragmentedIpPackets'],
                  api_lan['ipv4']['blocks']['portScanDetection'],
                  api_lan['ipv4']['blocks']['ipFlood'],
                  api_lan['ipv6']['active']
                  ]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append(
                '\n[Pass] 3. Check Value of APi IPv4: active, level, fragmentedIpPackets, portScanDetection, ipFlood; IPv6: active.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi IPv4: active, level, fragmentedIpPackets, portScanDetection, ipFlood; IPv6: active. \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check Value of APi IPv4: active, level, fragmentedIpPackets, portScanDetection, ipFlood; IPv6: active. \nActual: %s.  \nExpected: %s' % (
                str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_SFB_11] Assertion wrong')
...
class PageSecurityFirewallRemoteLog(unittest.TestCase):
    ...
    def test_UI_SFR_02(self):
        ...
        time.sleep(10)
        # Click OK
        ok_btn = driver.find_elements_by_css_selector('.msgBtn[id="ok"]')
        if len(ok_btn) != 0:
            ok_btn[-1].click()
        time.sleep(3)
        url_req = ipv4 + '/api/v1/service/log'
        token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]

        api_log = Helper.Helper_common.call_get_api_token(url_req, token)
        actual_values = [api_log[0]['active'],
                         api_log[0]['mode'],
                         api_log[0]['remote']['address'],
                         api_log[0]['remote']['port']]
    ...
    def test_UI_SFR_05(self):
        ...
        ActionChains(driver).move_to_element(apply_btn).click().perform()
        time.sleep(5)
        btn_ok = driver.find_elements_by_css_selector('#ok')
        if len(btn_ok) != 0:
            btn_ok[-1].click()
        time.sleep(10)
        # 3. The button works normally and apply setting successfully
    ...
    def test_UI_SFR_09(self):
        ...
        permittedConnection = Helper.Helper_common.check_radio_tick(driver, '#permittedConnection')
        blockedConnection = Helper.Helper_common.check_radio_tick(driver, '#blockedConnection')
        knownInternetAttack = Helper.Helper_common.check_radio_tick(driver, '#knownInternetAttack')
        productConfigEvent = Helper.Helper_common.check_radio_tick(driver, '#productConfigEvent')
        para_servidor = driver.find_element_by_css_selector('#para-servidor').text
        port = driver.find_element_by_css_selector('#port').get_attribute('value')
        actual = [permittedConnection,
                  blockedConnection,
                  knownInternetAttack,
                  productConfigEvent,
                  para_servidor,
                  port]
        expected = [False, False, False, False, '', '514']

        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('[Pass] 3.1 Value of Webui displayed.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.1 Value of Webui displayed wrong.\nActual: %s. \nExpected: %s'%(str(len(actual)), str(expected)))
            list_steps_fail.append('3.1 Value of Webui displayed wrong.\nActual: %s. \nExpected: %s'%(str(len(actual)), str(expected)))

        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/security/firewall/alert')
        actual = [api['syslogServer'],
                  api['option']['permittedConnections'],
                  api['option']['blockedConnections'],
                  api['option']['knownInternetAttacks'],
                  api['option']['productConfigurationEvent']]
        expected = ['', False, False, False, False]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of API firewall alert.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.2 Default value of API firewall alert.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.2 Default value of API firewall alert.\nActual: %s. \nExpected: %s.' % (actual, expected))

        self.assertListEqual(list_steps_fail, [], '[UI_SFR_09] Assertion Fail')
...
class PageSecurityIPFiltering(unittest.TestCase):
    ...
    def test_UI_SIF_02(self):
        ...
        for rule in rules:
            ...
            try:
                ...
                self.list_steps.append('[Pass] 3. Check radio box ticked: ' + str(tick))
            except AssertionError:
    ...
    def test_UI_SIF_05(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url)
            self.list_steps.append('\n[Pass] 2. Check Page Security IP Filter: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check Page Security IP Filter: ' + driver.current_url)
            list_steps_fail.append('2.URL page Security IP Filter display wrong' + driver.current_url)
        time.sleep(3)

        list_rules = driver.find_elements_by_css_selector('.table-fil tbody tr')
        try:
            self.assertEqual(len(list_rules), 0)
            self.list_steps.append('[Pass] 3.1 Filtragem por IP is empty.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.1 Filtragem por IP is existed.\nActual: %s. \nExpected: 0'%(str(len(list_rules))))
            list_steps_fail.append('3.1 Filtragem por IP is existed.\nActual: %s. \nExpected: 0'%(str(len(list_rules))))


        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/portIpFiltering')
        actual = [api['maxRules'], api['subnet']]
        expected = [10, '255.255.255.0']
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of API portIpFiltering.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.2 Default value of API portIpFiltering.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.2 Default value of API portIpFiltering.\nActual: %s. \nExpected: %s.' % (actual, expected))

        self.assertListEqual(list_steps_fail, [], '[UI_SIF_05] Assertion Fail')
...
class PageSecurityMACFiltering(unittest.TestCase):
    ...
    def test_UI_SMF_04(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url)
            self.list_steps.append('\n[Pass] 2. Check Page Security MAC Filter: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check Page Security MAC Filter: ' + driver.current_url)
            list_steps_fail.append('2.URL page Security MAC Filter display wrong' + driver.current_url)
        time.sleep(3)

        list_rules = driver.find_elements_by_css_selector('.list li')
        try:
            self.assertEqual(len(list_rules), 0)
            self.list_steps.append('[Pass] 3.1 Endereços inseridos is empty.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.1 Endereços inseridos is existed.\nActual: %s. \nExpected: 0'%(str(len(list_rules))))
            list_steps_fail.append('3.1 Endereços inseridos is existed.\nActual: %s. \nExpected: 0'%(str(len(list_rules))))


        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/macFiltering')
        actual = [api['maxRules']]
        expected = [20]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of API macFiltering.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.2 Default value of API macFiltering.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.2 Default value of API macFiltering.\nActual: %s. \nExpected: %s.' % (actual, expected))

        self.assertListEqual(list_steps_fail, [], '[UI_SMF_04] Assertion Fail')
...
class PageSecurityPortFiltering(unittest.TestCase):
    ...
    def test_UI_SPF_02(self):
        ...
        for rule in rules:
            ...
            try:
                ...
                self.list_steps.append('[Fail] 3. Check radio box ticked: ' + str(tick))
                list_steps_fail.append('3. Check radio box ticked: ' + str(tick))
    ...
    def test_UI_SPF_04(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url)
            self.list_steps.append('\n[Pass] 2. Check Page Security Port Filter: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check Page Security Port Filter: ' + driver.current_url)
            list_steps_fail.append('2.URL page Security Port Filter display wrong' + driver.current_url)
        time.sleep(3)

        list_rules = driver.find_elements_by_css_selector('.table-fil tbody tr')
        try:
            self.assertEqual(len(list_rules), 0)
            self.list_steps.append('[Pass] 3.1 Lista de Endereço filtragem is empty.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.1 Lista de Endereço filtragem is existed.\nActual: %s. \nExpected: 0'%(str(len(list_rules))))
            list_steps_fail.append('3.1  Lista de Endereço filtragem is existed.\nActual: %s. \nExpected: 0'%(str(len(list_rules))))


        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/portFiltering')
        actual = [api['maxRules']]
        expected = [20]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of API portFiltering.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.2 Default value of API portFiltering.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.2 Default value of API portFiltering.\nActual: %s. \nExpected: %s.' % (actual, expected))

        self.assertListEqual(list_steps_fail, [], '[UI_SPF_04] Assertion Fail')
...
class PageAdvancedPortForwarding(unittest.TestCase):
    ...
    def test_UI_APF_03(self):
        ...
        time.sleep(1)
        # Click Wifi
        driver.find_element_by_css_selector('[for=menu-avancado]').click()
        time.sleep(1)
        # Click Radio
        driver.find_element_by_css_selector('a[href="#page-advanced-port-forwarding"]').click()
        time.sleep(3)
        time.sleep(5)
        try:
            self.assertEqual(driver.current_url, ipv4 + '/#page-advanced-port-forwarding')
        ...
        try:
            ...
            self.list_steps.append("\n[Pass] 5.List config is empty")
        except AssertionError:
            self.list_steps.append("\n[Fail] 5.List config isn't empty")
            self.list_steps_fail.append("5.List config isn't empty")
        self.assertListEqual(self.list_steps_fail, [], "Assertion wrong")
    ...
    def test_UI_APF_05(self):
        ...
        # Check display the CRIAR button
        criar_btn = driver.find_element_by_css_selector("label.button.icon").get_attribute("value")
        actual = [criar_btn]
        expected = ['Criar']
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check display the CRIAR button.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check display the CRIAR button: \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append(
                '3. Check display the CRIAR button: \nActual: %s.  \nExpected: %s' % (str(actual), str(expected)))

        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/portForwarding')
        expected = [32, True]
        actual = [api_lan['maxRules'],
                  api_lan['active']]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3. Check Value of APi maxRules & active.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3. Check Value of APi maxRules & active.. \nActual: %s. \nExpected: %s' % (
                str(actual), str(expected)))
            list_steps_fail.append('3. Check Value of APi maxRules & active.. \nActual: %s.  \nExpected: %s' % (
            str(actual), str(expected)))

        self.assertListEqual(list_steps_fail, [], '[UI_APF_05] Assertion wrong')
...
class PageAdvancedPortTrigger(unittest.TestCase):
    ...
    def test_UI_APT_05(self):
        ...
        # Check response from API
        api_lan = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/portTriggering')
        expected = [32, True]
        actual = [api_lan['maxRules'],
                  api_lan['active']]
        try:
        ...
        self.assertListEqual(list_steps_fail, [], '[UI_APT_05] Assertion wrong')
...
class PageSecurityAccessByWifi(unittest.TestCase):
    ...
    def test_UI_SAB_02(self):
        ...
        visible = driver.find_elements_by_css_selector('[for="accessOff"]')
        count = 0
    ...
    def test_UI_SAB_04(self):
        ...
        ligado = Helper.Helper_common.check_radio_tick(driver, '#accessOn')
        try:
            self.assertTrue(ligado)
            self.list_steps.append('[Pass] 3.1 Access to Administration over Wifi is LIGADO')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.1 Access to Administration over Wifi is not LIGADO.')
            list_steps_fail.append('3.1  Access to Administration over Wifi is not LIGADO.')


        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/accessADM')
        actual = [api['accessADM']]
        expected = [True]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of API accessADM.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.2 Default value of API accessADM.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.2 Default value of API accessADM.\nActual: %s. \nExpected: %s.' % (actual, expected))

        self.assertListEqual(list_steps_fail, [], '[UI_SAB_04] Assertion Fail')
...
class PageSecurityParentalControl(unittest.TestCase):
    ...
    def test_UI_SPC_04(self):
        ...
        time.sleep(2)
        # Step 2: Access the "page-security-parental-control" page
        driver.get(ipv4 + '/#page-security-parental-control')
        time.sleep(3)
        try:
            self.assertEqual(driver.current_url, ipv4 + '/#page-security-parental-control')
        ...
        time.sleep(5)
        if len(driver.find_elements_by_css_selector("button#apply")) == 0 or len(
                driver.find_elements_by_css_selector('.msgText')) == 1:
            driver.refresh()
            time.sleep(5)

        # Step 3: Add 64 new parental control rules
        # Remove all parental control rules
        try:
            checkbox = driver.find_elements_by_css_selector("label.radio-check")
            if len(checkbox) > 0:
                for i in range(0, len(checkbox)):
                    ActionChains(driver).move_to_element(checkbox[i]).click().perform()

                btn_remove = driver.find_element_by_css_selector("button#removeParental")
                ActionChains(driver).move_to_element(btn_remove).click().perform()

            time.sleep(7)
            # if len(driver.find_elements_by_css_selector("button#apply")) == 0:
            #     driver.refresh()
            #     time.sleep(5)

            # Click add
            ActionChains(driver).move_to_element(driver.find_element_by_css_selector("button#apply")).click().perform()
            time.sleep(5)

            for i in range(1, 65):
                # Add rule name
                input_rule = driver.find_element_by_css_selector("input#filter-name")
                ActionChains(driver).move_to_element(input_rule).perform()
                input_rule.send_keys("Rule #" + str(i))
                time.sleep(1)

                # Click add rule
                ActionChains(driver).move_to_element(
                    driver.find_element_by_css_selector("div.holder-icon")).click().perform()

                time.sleep(5)
                if len(driver.find_elements_by_css_selector('.msgText')) == 1:
                    driver.refresh()
                    time.sleep(5)
                ...
                # Add more
                ActionChains(driver).move_to_element(
                    driver.find_element_by_css_selector("button#addParental")).click().perform()
                time.sleep(1)
            self.list_steps.append('\n[Pass] 3. The button work normally')
        except AssertionError:
            self.list_steps.append(
                "\n[Fail] 3. The button doesn't work normally")
            self.list_steps_fail.append("3. The button doesn't work normally")
        time.sleep(10)

        # Step 4: Add one more new parental control rule
        try:
            value_input = ['Rule #1', 'Rule #2', 'Rule #3', 'Rule #4', 'Rule #5', 'Rule #6', 'Rule #7', 'Rule #8',
                           'Rule #9', 'Rule #10', 'Rule #11', 'Rule #12', 'Rule #13', 'Rule #14', 'Rule #15',
                           'Rule #16', 'Rule #17', 'Rule #18', 'Rule #19', 'Rule #20', 'Rule #21', 'Rule #22',
                           'Rule #23', 'Rule #24', 'Rule #25', 'Rule #26', 'Rule #27', 'Rule #28', 'Rule #29',
                           'Rule #30', 'Rule #31', 'Rule #32', 'Rule #33', 'Rule #34', 'Rule #35', 'Rule #36',
                           'Rule #37', 'Rule #38', 'Rule #39', 'Rule #40', 'Rule #41', 'Rule #42', 'Rule #43',
                           'Rule #44', 'Rule #45', 'Rule #46', 'Rule #47', 'Rule #48', 'Rule #49', 'Rule #50',
                           'Rule #51', 'Rule #52', 'Rule #53', 'Rule #54', 'Rule #55', 'Rule #56', 'Rule #57',
                           'Rule #58', 'Rule #59', 'Rule #60', 'Rule #61', 'Rule #62', 'Rule #63', 'Rule #64']

            value_web = []
            get_rule = driver.find_elements_by_css_selector("strong.text")
            for i in range(0, len(get_rule)):
                ActionChains(driver).move_to_element(get_rule[i]).perform()
                value_web.append(get_rule[i].text)

            value_api = []
            token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]
            url_req = ipv4 + '/api/v1/security/managedDevices'
            api = Helper.Helper_common.call_get_api_token(url_req, token)[0]

            for i in range(0, len(api['devices'])):
                value_api.append(api['devices'][i]['name'])

            self.assertListEqual(value_input, value_web)
            self.list_steps.append('\n[Pass] 4. Values displayed as same as input')

            self.assertListEqual(value_web, value_api)
            self.list_steps.append('\n[Pass] 4. Values displayed as same as API')
        except AssertionError:
            self.list_steps.append(
                "\n[Fail] 4. Values haven't displayed as same as input and API")
            list_steps_fail.append("4. Values haven't displayed as same as input and API")

        # Step 5: Check input Rule #65
        try:
            # Add more
            ActionChains(driver).move_to_element(
                driver.find_element_by_css_selector("button#addParental")).click().perform()
            ...
            self.assertEqual(len(value_web), 64)
            self.list_steps.append('\n[Pass] 5. Can not input [Rule #65]')
        except AssertionError:
            self.list_steps.append(
                "\n[Fail] 5. Can input [Rule #65]")
            list_steps_fail.append("5. Can input [Rule #65]")
        self.assertListEqual(list_steps_fail, [], '[UI_SPC_04] Assertion wrong')
    ...
    def test_UI_SPC_06(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url)
            self.list_steps.append('\n[Pass] 2. Check Page Network Parental control: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check Page Network Parental control: ' + driver.current_url)
            list_steps_fail.append('2.URL page Network Parental control display wrong' + driver.current_url)
        time.sleep(3)

        button = driver.find_element_by_css_selector('#apply>div').text
        try:
            self.assertIn('ADICIONAR FILTRO', button)
            self.list_steps.append('[Pass] 3.1 Button ADICIONAR FILTRO displayed in web page')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.1 Button ADICIONAR FILTRO did not display in web page.')
            list_steps_fail.append('3.1 Button ADICIONAR FILTRO did not display in web page.')

        self.assertListEqual(list_steps_fail, [], '[UI_SPC_06] Assertion Fail')
...
class PageWifiPrimaryNetwork(unittest.TestCase):
    ...
    def test_UI_WPN_10(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url_target)
            self.list_steps.append('\n[Pass] 2. Check URL of Page Wifi Primary Network: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check URL of Page Wifi Primary Network: ' + driver.current_url)
            list_steps_fail.append(
                '2. URL Page Wifi Primary Network wrong: Actual: ' + driver.current_url + ' Expected: ' + expected_url_target)
        time.sleep(4)

        interface = Helper.Helper_common.check_radio_tick(driver, '[id="2-4-ghz"]')

        ativar = Helper.Helper_common.check_radio_tick(driver, '[id="active"]')

        network_name = driver.find_element_by_id('network_name').get_attribute('value')

        value = driver.find_element_by_css_selector('#close_network').get_attribute('value')
        text1 = driver.find_element_by_css_selector('#close_network > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#mode_required').get_attribute('value')
        text2 = driver.find_element_by_css_selector('#mode_required > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#ap_isolate').get_attribute('value')
        text3 = driver.find_element_by_css_selector('#ap_isolate > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#security_type').get_attribute('value')
        text4 = driver.find_element_by_css_selector('#security_type > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#encryption').get_attribute('value')
        text5 = driver.find_element_by_css_selector('#encryption > option[value="' + value + '"]').text

        mostrar = Helper.Helper_common.check_radio_tick(driver, '[id="show_wpa"]')

        group_key = driver.find_element_by_id('group-key').get_attribute('value')

        habilitar = Helper.Helper_common.check_radio_tick(driver, '[id="habilitar"]')

        actual = [interface, ativar, network_name, text1, text2, text3, text4, text5, mostrar, group_key, habilitar]
        expected = [True, True, name_2g, 'Desligado', 'Nenhum', 'Desativado', 'WPA/WPA2-PSK', 'AES/TKIP', False, '3600', True]

        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.1 Value of default webui displayed. ')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.1 Value of default webui displayed wrong. \nACtual: %s. \nExpected: %s' %(str(actual), str(expected)))
            list_steps_fail.append('3.1 Value of default webui displayed wrong. \nACtual: %s. \nExpected: %s' %(str(actual), str(expected)))

        token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]
        url_req = ipv4 + '/api/v1/wifi/0/ssid/0'
        api = Helper.Helper_common.call_get_api_token(url_req, token)[0]

        actual = [api['active'],
                  api['security']['type'],
                  api['security']['personal']['groupKey'],
                  api['security']['personal']['encryption'],
                  api['hiddenSSID'],
                  api['APIsolate']]
        expected = [True, "WPA2/WPA-PSK", 3600, "AES/TKIP", False, False]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of API SSID.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.2 Default value of API SSID.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.2 Default value of API SSID.\nActual: %s. \nExpected: %s.' % (actual, expected))

        url_req = ipv4 + '/api/v1/wifi/wps?interfaceId=0'
        api = Helper.Helper_common.call_get_api_token(url_req, token)[0]
        actual = [api['active']]
        expected = [True]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.3 Default value of API WPS.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.3 Default value of API WPS.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.3 Default value of API WPS.\nActual: %s. \nExpected: %s.' % (actual, expected))
        self.assertListEqual(list_steps_fail, [], '[UI_WPN_10] Assertion Fail')
...
class PageWifiWdsConfiguration(unittest.TestCase):
    ...
    def test_UI_WWC_02(self):
        ...
        # Step 4. Click "RECARREGAR " button to restore default WDS configurations and apply settings
        try:
            reset_btn = driver.find_element_by_css_selector("button[value='reset']>div.holder-icon")
            ActionChains(driver).move_to_element(reset_btn).click().perform()
            time.sleep(3)

            token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]
            url_req = ipv4 + '/api/v1/wifi/0/wds'
            api = Helper.Helper_common.call_get_api_token(url_req, token)[0]
            self.assertEqual(api['active'], False)
            self.assertEqual(len(api['rules']), 0)

            self.list_steps.append('\n[Pass] 4. Values display as same as response from API')
        except AssertionError:
            self.list_steps.append('\n[Fail] 4. Values did not display as same as response from API')
            self.list_steps_fail.append('4. Values did not display as same as response from API')

        # Step 5: Click "5 GHZ" to select 5 Ghz interface
        try:
            driver.find_element_by_css_selector('.custom-radio[for="5-ghz"]').click()
            time.sleep(2)
            self.list_steps.append('\n[Pass] 5. The button works normally')
        ...
        # Step 6. Click "RECARREGAR " button to restore default WDS configurations and apply settings
        try:
            reset_btn = driver.find_element_by_css_selector("button[value='reset']>div.holder-icon")
            ActionChains(driver).move_to_element(reset_btn).click().perform()
            time.sleep(3)
            ...
            self.list_steps.append('\n[Pass] 6. Values display as same as response from API')
        except AssertionError:
            self.list_steps.append('\n[Fail] 6. Values did not display as same as response from API')
            self.list_steps_fail.append('6. Values did not display as same as response from API')

        self.assertEqual(self.list_steps_fail, [], '[UI_WWC_02] Assertion wrong')
    ...
    def test_UI_WWC_04(self):
        ...
        driver.find_element_by_css_selector('a[href="#page-wifi-wds-configuration"]').click()
        expected_url_target = ipv4 + '/#page-wifi-wds-configuration'
        time.sleep(3)
        try:
            self.assertEqual(driver.current_url, expected_url_target)
            self.list_steps.append('\n[Pass] 2. Check URL of Page Wifi WDS Configuration : ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check URL of Page Wifi WDS Configuration: ' + driver.current_url)
            list_steps_fail.append(
                '2. URL Page Wifi WDS Configuration wrong: Actual: ' + driver.current_url + ' Expected: ' + expected_url_target)
        time.sleep(1)

        interface = Helper.Helper_common.check_radio_tick(driver, '[id="2-4-ghz"]')
        try:
            self.assertTrue(interface)
            self.list_steps.append('\n[Pass] 3.1 Interface 2G is checked in default. ')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.1 Interface 2G is not  checked in default. ')
            list_steps_fail.append('3.1 Interface 2G is not  checked in default')

        bridge = driver.find_element_by_css_selector('.combo-box#bridge').get_attribute('value')
        bridge_actual = driver.find_element_by_css_selector('.combo-box#bridge > option[value="'+bridge+'"]').text
        bridge_expected = 'Desativado'
        try:
            self.assertEqual(bridge_actual, bridge_expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of Bridge.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.2 Default value of Bridge.\nActual: %s. \nExpected: %s.' % (
            bridge_actual, bridge_expected))
            list_steps_fail.append(
                '3.2 Default value of Bridge.\nActual: %s. \nExpected: %s.' % (bridge_actual, bridge_expected))

        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/wifi/0/wds')
        actual = [api['active'], api['rules']]
        expected = [False, []]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.3 Default value of API WDS.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.3 Default value of API WDS.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.3 Default value of API WDS.\nActual: %s. \nExpected: %s.' % (actual, expected))

        self.assertListEqual(list_steps_fail, [], '[UI_WWC_04] Assertion Fail')
...
class PageWifiRadio(unittest.TestCase):
    ...
    #     expected = [True, 'MEXICO', 'high', 'auto', '40', 'auto', 'false', True, '80']
    #     try:
    ...
    #         # self.assertEqual(band_with_2g, '40')
    #         # self.assertEqual(chanel, 'auto')
    #         # self.assertEqual(beamforming, 'false')
    #         # self.assertEqual(ativar_tick_5g, True)
    #         # self.assertEqual(band_with_5g, '80')
    #         self.assertListEqual(list_actual_value, expected)
    #         self.list_steps.append('\n[Pass] 4.2 Check WebUI displayed: \nActual: %s. \nExpected: %s'%(str(list_actual_value), str(expected)))
    #     except AssertionError:
    #         self.list_steps.append('\n[Fail] 4.2 Check WebUI displayed: \nActual: %s. \nExpected: %s'%(str(list_actual_value), str(expected)))
    #         list_steps_fail.append('4.2 WebUI displayed wrong: \nActual: %s. \nExpected: %s'%(str(list_actual_value), str(expected)))
    #
    ...
    def test_UI_WR_02(self):
        ...
        try:
            ...
            self.list_steps.append('\n[Pass] 3. Check API uncheck Ativar return: \nActual: %s. \nExpected: %s.'%(str(actual), str(False)))
        except AssertionError:
            self.list_steps.append('\n[Fail] 3. Check API uncheck Ativar return: \nActual: %s. \nExpected: %s.'%(str(actual), str(False)))
            list_steps_fail.append('3. API uncheck Ativar Page Wifi Radio wrong: \nActual: %s. \nExpected: %s.'%(str(actual), str(False)))
    ...
    def test_UI_WR_09(self):
        ...
        url_req = ipv4 + '/api/v1/wifi/1/radio'
        api_wifi_radio = Helper.Helper_common.call_get_api_token(url_req, token)[0]
    ...
    def test_UI_WR_16(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url_target)
            self.list_steps.append('\n[Pass] 2. Check URL of Page Wifi Radio: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check URL of Page Wifi Radio: ' + driver.current_url)
            list_steps_fail.append(
                '2. URL Page Wifi Radio wrong: Actual: ' + driver.current_url + ' Expected: ' + expected_url_target)
        time.sleep(4)

        interface = Helper.Helper_common.check_radio_tick(driver, '[id="radio2g"]')

        ativar = Helper.Helper_common.check_radio_tick(driver, '[id="active"]')

        country = driver.find_element_by_id('country').get_attribute('readonly')

        value = driver.find_element_by_css_selector('#outputPower').get_attribute('value')
        text1 = driver.find_element_by_css_selector('#outputPower > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#wirelessMode').get_attribute('value')
        text2 = driver.find_element_by_css_selector('#wirelessMode > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#bandwidth').get_attribute('value')
        text3 = driver.find_element_by_css_selector('#bandwidth > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#sideband').get_attribute('value')
        text4 = driver.find_element_by_css_selector('#sideband > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#channel').get_attribute('value')
        text5 = driver.find_element_by_css_selector('#channel > option[value="' + value + '"]').text

        value = driver.find_element_by_css_selector('#beamforming').get_attribute('value')
        text6 = driver.find_element_by_css_selector('#beamforming > option[value="' + value + '"]').text



        actual = [interface, ativar, country, text1, text2, text3, text4, text5, text6]
        expected = [True, True, 'true', 'Alto', 'Auto', '40MHz', 'Baixo', 'Auto', 'Disabled']
        ...
        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/service/networkOption')
        actual = [api['wanBlocking'],
                  api['ipsecPassthrough'],
                  api['pptpPassthrough'],
                  api['l2tpPassthrough'],
                  api['multicast'],
                  api['zeroConfiguration']]
        expected = [True, False, False, False, True, False]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of API networkOption.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.2 Default value of API networkOption.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.2 Default value of API networkOption.\nActual: %s. \nExpected: %s.' % (actual, expected))

        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/wifi/0/radio')
        actual = [api['active'],
                  api['basic']['channel']['set'],
                  api['basic']['channel']['used'],
                  api['basic']['wirelessMode'],
                  api['basic']['bandwidth']['set'],
                  api['basic']['bandwidth']['used'],
                  api['basic']['sideband'],
                  api['basic']['outputPower'],
                  api['advanced']['beamforming']]
        expected = [True, 'auto', '1', "802.11b+g+n", '40', '20', 'lower', 'high', False]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.3 Default value of API Radio.')
        except AssertionError:
            self.list_steps.append(
                '\n[Fail] 3.3 Default value of Radio WPS.\nActual: %s. \nExpected: %s.' % (actual, expected))
            list_steps_fail.append(
                '3.3 Default value of Radio WPS.\nActual: %s. \nExpected: %s.' % (actual, expected))
        self.assertListEqual(list_steps_fail, [], '[UI_WR_16] Assertion Fail')
...
class PageWifiConnectedEquip(unittest.TestCase):
    ...
    def test_UI_WCE_01(self):
        ...
        url_req = ipv4 + '/api/v1/wifi/0/ssid/0/accessControl'
        token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]

        api_wifi_access_control = Helper.Helper_common.call_get_api_token(url_req, token)[0]
        ...
        ActionChains(driver).move_to_element(apply_btn).click().perform()
        Helper.Helper_common.wait_time(self, driver)
        api_wifi_access_control = Helper.Helper_common.call_get_api_token(url_req, token)
        try:
    ...
    def test_UI_WCE_02(self):
        ...
        url_req = ipv4 + '/api/v1/wifi/1/ssid/0/accessControl'
        token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]
        api_wifi_access_control = Helper.Helper_common.call_get_api_token(url_req, token)[0]
    ...
    def test_UI_WCE_05(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url_target)
            self.list_steps.append('\n[Pass] 2. Check URL of Page Wifi Connnected equip: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check URL of Page Wifi Connnected equip: ' + driver.current_url)
            list_steps_fail.append(
                '2. URL Page Wifi Connnected equip wrong: Actual: ' + driver.current_url + ' Expected: ' + expected_url_target)
        time.sleep(1)

        interface = Helper.Helper_common.check_radio_tick(driver, '[id="2g"]')
        try:
            self.assertTrue(interface)
            self.list_steps.append('\n[Pass] 3.1 Interface 2G is checked in default. ')
        except AssertionError:
        ...
        mac = driver.find_element_by_css_selector('#mac-permission').get_attribute('value')
        mac_actual = driver.find_element_by_css_selector('#mac-permission>option[value="'+mac+'"]').text
        mac_expected = 'Desativado'
        try:
            self.assertEqual(mac_actual, mac_expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of Mac Permission.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.2 Default value of Mac Permission.\nActual: %s. \nExpected: %s.'%(mac_actual, mac_expected))
            list_steps_fail.append('3.2 Default value of Mac Permission.\nActual: %s. \nExpected: %s.'%(mac_actual, mac_expected))

        api = Helper.Helper_common.call_get_api_genaeral('/api/v1/wifi/0/ssid/0/accessControl')
        actual = [api['active'], api['allow'], api['maxRules'], api['probeResponse']]
        expected = [False, False, 20, False]
        try:
            self.assertListEqual(actual, expected)
            self.list_steps.append('\n[Pass] 3.2 Default value of API AccessControl.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.2 Default value of API AccessControl.\nActual: %s. \nExpected: %s.'%(actual, expected))
            list_steps_fail.append('3.2 Default value of API AccessControl.\nActual: %s. \nExpected: %s.'%(actual, expected))

        api2 = Helper.Helper_common.call_get_api_genaeral('/api/v1/gateway/devices?connected=true&interface=2.4g')
        try:
            self.assertListEqual(api2, [])
            self.list_steps.append('\n[Pass] 3.2 Default value of API Devices.')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3.2 Default value of API Devices.\nActual: %s. \nExpected: %s.'%(str(api2), str([])))
            list_steps_fail.append('3.2 Default value of API Devices.\nActual: %s. \nExpected: %s.'%(str(api2), str([])))

        self.assertListEqual(list_steps_fail, [], '[UI_WCE_05] Assertion Fail')
...
class PageAdminRouterPassword(unittest.TestCase):
    ...
    def test_UI_ARP_01(self):
        ...
        Helper.Helper_common.wait_time(self, driver)
        #
        os.system('python run_start.py ' + model + ' ' + rg_port + ' ' + cm_port)
        # os.system(
        ...
        #
        os.system(
                '''"C:\Program Files\VanDyke Software\SecureCRT\SecureCRT.exe" /SCRIPT ./factory.py /SERIAL '''
                + com + ''' /BAUD 115200''')
        time.sleep(150)
        os.system('python run_start.py ' + model + ' ' + rg_port + ' ' + cm_port)
        time.sleep(3)
...
class PageAdminDiagnostics(unittest.TestCase):
    ...
    def test_UI_ADI_01(self):
        ...
        url_req = ipv4 + '/api/v1/service/diagnostics/ping'
        token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]
        body = {
            "target": ip,
            "size": size,
        ...
        api_diagnostics_ping = Helper.Helper_common.call_post_api(url_req=url_req, token=token, body=body)

        # api_diagnostics_ping = Helper.Helper_common.api_diagnostics_ping(ip, int(size), int(count), int(interval))
        if not len(api_diagnostics_ping[0]['msg']):
            result_expected = ''
        else:
            result_expected = ''
            for i in api_diagnostics_ping[0]['msg']:
                result_expected += i
        ...
        self.assertListEqual(list_steps_fail, [], '[UI_ADI_01] Assertion Ping diagnostic fail')
    ...
    def test_UI_ADI_02(self):
        ...
        Helper.Helper_common.wait_time(self, driver)
        time.sleep(5)
        result_actual = driver.find_element_by_css_selector('#result').text

        token = Helper.Helper_common.call_api_login(user, pass_word)[0]["accessToken"]
        url_req = ipv4 + '/api/v1/service/diagnostics/routehops'
        api_diagnostics_routehops = Helper.Helper_common.call_post_api(url_req, token)[0]
        ...
        if not len(api_diagnostics_routehops['msg']):
            ...
            for i in api_diagnostics_routehops['msg']:
                result_expected += i
        hop = [i for i in api_diagnostics_routehops['msg'] if i.startswith("Hop")]
        try:
            self.assertIn(result_actual, result_expected)
            self.list_steps.append('\n[Pass] 4. Check API return from Return box: ' + result_actual)
        except AssertionError:
            self.list_steps.append('\n[Fail] 4. Check API return from Return box: \nActual: %s. \nExpected: %s.' %(result_actual, str(hop)))
            list_steps_fail.append('4. API return from Return box wrong: \nActual: %s. \nExpected: %s.' %(result_actual, str(hop)))

        self.assertListEqual(list_steps_fail, [], '[UI_ADI_02] Assertion Traceroute diagnostic fail')
...
class PageAdminBackup(unittest.TestCase):
    ...
    #     app.Open.Edit.SetText(file_path)
    #     time.sleep(2)
    #     app.Open.Open.click()
    #     time.sleep(2)
    #     # Click Restaurar
...
class PageAdminFactoryDefault(unittest.TestCase):
    ...
    def test_UI_AFD_01(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url_target)
            self.list_steps.append('\n[Pass] 4.2 Check URL of Page Admin Factory Default: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 4.2 Check URL of Page Admin Factory Default: ' + driver.current_url)
            list_steps_fail.append('4.2 URL Page Admin Factory Default wrong: ' + driver.current_url)
        time.sleep(3)
        # Click SIM
        ...
        self.assertListEqual(list_steps_fail, [], '[UI_AFD_01] Assertion Ping diagnostic fail')
    ...
    def test_UI_AFD_03(self):
        ...
        try:
            self.assertEqual(driver.current_url, expected_url_target)
            self.list_steps.append('\n[Pass] 2. Check URL of Page Admin factory Default: ' + driver.current_url)
        except AssertionError:
            self.list_steps.append('\n[Fail] 2. Check URL of Page Admin factory Default: ' + driver.current_url)
            list_steps_fail.append(
                '2. URL Page Admin factory Default wrong: Actual: ' + driver.current_url + ' Expected: ' + expected_url_target)
        time.sleep(1)

        nao = Helper.Helper_common.check_radio_tick(driver, '#restaurar-de-fabrica-nao')
        try:
            self.assertTrue(nao)
            self.list_steps.append('\n[Pass] 3. NAO is checked in default. ')
        except AssertionError:
            self.list_steps.append('\n[Fail] 3. NAO is not  checked in default. ')
            list_steps_fail.append('3. NAO is not  checked in default')

        self.assertListEqual(list_steps_fail, [], '[UI_AFD_03] Assertion Fail')


if __name__ == '__main__':
    unittest.main()
    # suite = unittest.TestLoader().loadTestsFromModule(PageLogin)
