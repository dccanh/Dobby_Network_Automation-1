...
class PageLogin(unittest.TestCase):
    ...
    #     os.system(
    #         '''"C:\Program Files\VanDyke Software\SecureCRT\SecureCRT.exe" /SCRIPT ./restore_default.py /SERIAL ''' + com + ''' /BAUD 115200''')
    #     time.sleep(180)
    ...
    def test_UI_LG_02(self):
        ...
        driver.maximize_window()
        # Log in
        Helper.Helper_common.check_login(driver, self, ipv6)
        time.sleep(3)
        driver.find_element_by_id('login').send_keys(user)
        driver.find_element_by_id('senha').send_keys(pass_word)
        driver.find_element_by_xpath('//button[@value="Entrar"]').click()
    ...
    def test_UI_LG_03(self):
        ...
        driver.maximize_window()
        # Log in
        Helper.Helper_common.check_login(driver, self, ipv6_global)
        time.sleep(3)
        driver.find_element_by_id('login').send_keys(user)
        driver.find_element_by_id('senha').send_keys(pass_word)
        driver.find_element_by_xpath('//button[@value="Entrar"]').click()
    ...
    def test_UI_LG_05(self):
        ...
        while len(pop_up_wait) == 1:
            ...
            if count_time >= 300:
                break
        self.assertLessEqual(count_time, 300, '[UI_LG_05] Pop-up change PW was timeout > 5 minutes')
    ...
    def test_UI_LG_04(self):
        ...
        try:
            self.assertEqual(16, password_length)
            self.assertIn('Senha incorreta', driver.page_source)
            self.list_steps.append("\n[Pass] 1. Lengh password = 16, 'Senha incorreta' message is displayed")
        except AssertionError:
            self.list_steps.append("\n[Fail] 1. Lengh password != 16, 'Senha incorreta' message is not displayed")
            list_steps_fail.append('[UI_LG_04] Verify Login with correct user name and wrong password fail')
            pass

        # Step 2: Log in with correct password and wrong username
        driver.get(ipv4 + '/#page-login')
        time.sleep(1)
        ...
        try:
            self.assertEqual(32, username_length)
            self.assertIn('Login incorreto', driver.page_source)
            self.list_steps.append("\n[Pass] 2. Lengh username = 32, 'Login incorreto' message is displayed")
        except AssertionError:
            self.list_steps.append("\n[Fail] 1. Lengh password != 32, 'Login incorreto' message is not displayed")
            list_steps_fail.append('[UI_LG_04] Verify Login with wrong user name and correct password fail')
            pass
...
class PageQuickSetup(unittest.TestCase):
    ...
    def test_UI_QS_06(self):
        ...
        try:
            ...
            list_steps_fail.append('3.1 Name display wrong: ' + str(actual_name))
            pass
...
# 1 2
class NetworkBasicSettings(unittest.TestCase):
    def setUp(self):
        self.list_steps = []
        self.start_time = datetime.datetime.now()
        self.driver = webdriver.Chrome('../Driver/chromedriver.exe')
    ...
    def test_UI_NBS_01(self):
        ...
        # verify data
        try:
            self.assertEqual(ipv4_address, ipv4_address_expected, "[UI_NBS_01] IPv4 Address isn't correct")
            self.assertEqual(ipv6_address, ipv6_address_expected, "[UI_NBS_01] IPv6 Address isn't correct")
            self.assertEqual(ipv6_prefix, ipv6_prefix_expected, "[UI_NBS_01] IPv6 Prefix isn't correct")
            self.assertEqual(1, mac_address_expected, "[UI_NBS_01] MAC Address isn't correct")
            self.list_steps.append(
                '3. ' + ipv4_address + '\n' + ipv6_address + '\n' + ipv6_prefix + '\n' + mac_address)

        except AssertionError:
            self.list_steps.append('3. ' + driver.current_url)
            self.list_steps.append(
                '3. ' + ipv4_address + '\n' + ipv6_address + '\n' + ipv6_prefix + '\n' + mac_address)
    ...
    def test_UI_NBS_02(self):
        ...
        # verify data
        self.assertEqual(ipv4_address, ipv4_address_expected, "[UI_NBS_02] IPv4 Address isn't correct")
        self.assertEqual(ipv6_address, ipv6_address_expected, "[UI_NBS_02] IPv6 Address isn't correct")
        self.assertEqual(total_connection_time, total_connection_time_expected,
                         "[UI_NBS_02] Total connection time isn't correct")
        self.assertEqual(expiration_time, expiration_time_expected, "[UI_NBS_02] Expiration time isn't correct")
        self.assertEqual(IPv4_DNS_Server, IPv4_DNS_Server_expected, "[UI_NBS_02] IPv4 DNS Server isn't correct")
        self.assertEqual(IPv6_DNS_Server, IPv6_DNS_Server_expected, "[UI_NBS_02] IPv6 DNS Server isn't correct")
        self.assertEqual(mac_address, mac_address_expected, "[UI_NBS_02] MAC Address isn't correct")
...
class PageAdvancedPortForwarding(unittest.TestCase):
    def setUp(self):
        ...
        driver = self.driver
        driver.maximize_window()
        # Helper.Helper_common.login(driver, ipv4)
        self.list_steps = []
...
class PageWifiPrimaryNetwork(unittest.TestCase):
    ...
    def test_UI_WPN_05(self):
        ...
        expected_password = Common.Common.base64encode(
...
class PageWifiRadio(unittest.TestCase):
    ...
    def test_UI_WR_15(self):
        ...
        # Configuration Advance
        driver.find_element_by_css_selector('.next.config').click()
        expected_url_target = ipv4 + '/#page-status-so9ftware'
        try:
            self.assertEqual(driver.current_url, expected_url_target)
            self.list_steps.append('\n[Pass] 2.1 Check URL of Configuration Advance: ' + driver.current_url)
        except AssertionError:
        ...
        time.sleep(10)
        os.system(
            '''"C:\Program Files\VanDyke Software\SecureCRT\SecureCRT.exe" /SCRIPT ./wl_config.py /SERIAL ''' + com + ''' /BAUD 115200''')
        expected_uncheck = ['down', '0x0001', '0', 'value=0']
        config = configparser.ConfigParser()
        config.read_file(open(r'../Config/config.txt'))
        actual_uncheck = config.get('WIRELESS', 'value')
        actual_uncheck = actual_uncheck.split()
        try:
            ...
            list_steps_fail.append('5. Value return RG console when Uncheck ativar return wrong')
            pass

        time.sleep(7)
        wifi_list = str(subprocess.check_output('netsh wlan show networks', shell=True))
        list_wf_ssid = re.findall(r'SSID \d+ : [^\n ]*', wifi_list)
        try:
            self.assertNotIn(name_2g, list_wf_ssid)
            self.list_steps.append('\n[Pass] 6. Check 2G SSID in wifi list ')
        except AssertionError:
            self.list_steps.append('\n[Fail] 6. Check 2G SSID in wifi list ')
            list_steps_fail.append(' 6. 2G SSID is not in wifi list: ')
            pass
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Check Ativar
        if Helper.Helper_common.check_radio_tick(driver, '.radio-check-controler') != 'true':
        ...
        os.system(
            '''"C:\Program Files\VanDyke Software\SecureCRT\SecureCRT.exe" /SCRIPT ./wl_config.py /SERIAL ''' + com + ''' /BAUD 115200''')
        expected_check = ['up', '0x0000', '1', 'value=1']
        config = configparser.ConfigParser()
        config.read_file(open(r'../Config/config.txt'))
        actual_check = config.get('WIRELESS', 'value')
        ...
        try:
            ...
            list_steps_fail.append('8. Value return RG console when check ativar return wrong')
            pass
        ...
        list_wf_ssid = re.findall(r'SSID \d+ : [^\n ]*', wifi_list)
        try:
            self.assertIn(name_2g, list_wf_ssid)
            self.list_steps.append('\n[Pass] 9. Check 2G SSID in wifi list ')
        except AssertionError:
            self.list_steps.append('\n[Fail] 9. Check 2G SSID in wifi list ')
            list_steps_fail.append(' 9. 2G SSID is not in wifi list: ')
            pass

        self.assertListEqual(list_steps_fail, [], '[AU_WF_01] Assertion 2.4GHz wifi radio verification fail')
